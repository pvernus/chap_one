...
# select variables
    dplyr::select(1:6, 8:26, 44, 45, 59, 46:49, 50:53, 74:77) |>
...
```{r submission}
df <- filter(df, !initial_report %in% c(2,5,9))
```
...
# copy
df <- copy(data_channel_level)

# merge w/ data set
df <- left_join(df, delivery_crs_code,
...
data_iso3c <- data_crs |> select(country = recipient_name, iso3c) |> unique() |> arrange(country)
