...
# NOTE: years < 2008 have the higher proportions of missing channels
  ...
  data_channel_code |> 
    ...
    summarize(amount = sum(usd_commitment_defl, na.rm = T), .by = rep) |> 
...
#    filter(year < 2008) |>  
    ...
    mutate(na = ifelse( 
      ...
      1, 0)) |> 
    summarize(amount = sum(usd_commitment_defl, na.rm = T), .by = na) |> 
    mutate(total_amount=sum(amount)) |> 
    mutate(share=amount/total_amount)
...
data_crs |> 
  ...
  summarize(n = sum(usd_commitment_defl, na.rm = T), .by = report) |> 
  mutate(total = sum(n)) |>
...
data_crs |> 
  ...
  mutate(report = fct(ifelse(!is.na(channel_reported_name), "Yes", "No"))) |> 
  filter(report=="No") |>
  summarize(n = sum(usd_commitment_defl, na.rm = T), .by = aid_t) |> 
  mutate(total = sum(n)) |>
  mutate(share = round(n / total * 100, 2)) |>
  arrange(desc(n))
...
data_crs |> 
  ...
  mutate(report = fct(ifelse(!is.na(channel_reported_name), "Yes", "No"))) |> 
  filter(report=="No") |>
  summarize(n = sum(usd_commitment_defl, na.rm = T), .by = c(report, aid_t) |> 
  mutate(total = sum(n)) |>
  mutate(share = round(n / total * 100, 2)) |>
  arrange(desc(n))
```
...
## save
save(data_iso3c, data_unit, data_unit_hum,
