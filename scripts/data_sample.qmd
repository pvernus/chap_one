...
## Unique and binary treatment, staggered setting, hum/dev commitments

```{r att_bin_stagg}

# treatment
treat_first <- select(data_first,
         iso3c, year, d.name = first_treat_dis)
treat_first_cohort <- select(data_cohort,
         iso3c, year, g.name = cohort_dis)
treatment <- merge(treat_first, treat_first_cohort, by = c("iso3c", "year"))
# outcome
outcome <- data_outcome_hum |> 
  select(donor_id, recipient_id, year, sector_hum, y.name = commit_total) |> 
  filter(year >= 2004) |> 
  summarize(
