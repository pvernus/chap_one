...
summarize(data_clim,
  ...
  labs(
    x = "",
    y = "Average # of days > historical mean + sd"
  )
...
summarize(data_clim,
  ...
  labs(
    x = "",
    y = "Proportion of extreme events (days > historical mean + sd)"
  )
...
load(file = "C:\\Users\\pauvernu\\Seafile\\library\\chap_one_data\\data\\outcome\\data_iso3c.RData")
...
anti_join(distinct(data_gdis_short, disasterno, disastertype),
          ...
          by = "disasterno")
# NOTE: 125 events in GDIS w/out a match in EMDAT
...
length(unique(join_gdis_emdat$geo_id))

setdiff(data_clim$geo_id, join_gdis_emdat$geo_id)
data_gdis |> 
  filter(geo_id %in% setdiff(data_clim$geo_id, join_gdis_emdat$geo_id))
# NOTE: geo_id in data_clim but missing in join_gdis_emdat are located in (i) Slovenia (not included in OECD CRS iso3c) and (ii) Ukraine (location in Crimea, that GDIS and EM-DAT code either in Ukraine or in Russia, so I removed it from the sample, see above).

setdiff(join_gdis_emdat$geo_id, data_clim$geo_id)
data_gdis |> 
  filter(geo_id %in% setdiff(join_gdis_emdat$geo_id, data_clim$geo_id)) |> 
  select(country, iso3c, geo_id, disasterno, disastertype) |> 
  slice_sample(n = 10)

merged_df <- left_join(data_clim, join_gdis_emdat,
          by = "geo_id",
          suffix = c('', '_dis'))

# CHECK: object should have the same number of obs. as data_clim
length(unique((merged_df$geo_id)))
length(unique((data_clim$geo_id)))
...
data_severity <- filter(merged_df, !(disaster_type_gdis == "extreme temperature" & is.na(disaster_type_emdat)))
```
