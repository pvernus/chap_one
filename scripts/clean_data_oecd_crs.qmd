...
folder_path <- here('source', 'outcome', 'crs')
...
# create a data.table
data_combined <- data.table::rbindlist(data_list, fill = T)
```
...
data_short <- data_combined |> 
  dplyr::select(1:26, 39, 44:54, 57:59, 62:67, 74:77, 78) |> # dplyr::select relevant vars
  mutate_if(is.character, str_trim) # delete white space before/after text
```
...
][flow_code %in% c(11, 13),]
```

**Cooperation modalities**

-   **Remove**: project observations in provider countries e.g. (E) Scholarships and student costs in donor countries, (F) Debt Relief, (G) Administrative costs not included elsewhere, and (H) Other in-donor expenditures.
-   **Keep**: (A) Budget support, (B) Core contributions and pooled programmes and funds, (C) Project-type interventions, (D) Experts and other technical assistance.
...
-   **Remove**: non-specified country/region.

-   **Keep**: recipient countries.
...
-   **Remove**: (i) observations with missing/null value for both commitment or disbursement variables; (ii) non-concessional official & private flows.

-   **Keep**: grants and loans ODA.
...
# remove
rm(file_list, data_list, data_combined, data_short)
```
...
delivery_crs_code <- data.table(read_xlsx(here("docs", "OECD-ODA-Single-Table-2024-for-2023-flows.xlsx"),
  ...
  dplyr::select(channel_parent_code_2024 = channel_parent_category, 
...
data_channel <- data_channel |> 
  mutate(
    channel_state = if_else(channel_parent_code_2024 %in% c(10000:12000), 1, 0),
    channel_nonstate = if_else(channel_parent_code_2024 %in% c(20000:63000), 1, 0),
    channel_nonstate_ngo = if_else(channel_parent_code_2024 %in% c(20000:23000, 51000), 1, 0),
    channel_nonstate_multi = if_else(channel_parent_code_2024 %in% c(40000:47000), 1, 0),
    channel_nonstate_priv = if_else(channel_parent_code_2024 %in% c(30000:32000, 60000:63000), 1, 0),
...
data_channel |> 
  mutate(channel_multin = case_when(
    channel_parent_code_2024 %in% c(10000:12000) ~ "Public sector",
    channel_parent_code_2024 %in% c(20000:23000, 51000) ~ "NGOs and Civil society",
    channel_parent_code_2024 %in% c(40000:47000) ~ "Multilaterals",
    channel_parent_code_2024 %in% c(30000:32000, 60000:63000) ~ "Private sector",
    .default = "Other"
  ))
